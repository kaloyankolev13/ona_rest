    ...
    new mapboxgl.Marker({ element: el, anchor: "bottom" })
      ...
      .setPopup(
        new mapboxgl.Popup({ offset: 25, closeButton: false }).setHTML(
          '<strong style="color:#072b16">ONA</strong><br/>Стакевци, ул. 31 №1'
        )
      )
      .addTo(map);
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (infoRef.current) {
        ...
        if (blocks.length) { animateRevealOnScroll(blocks, { y: 40, duration: 0.6, stagger: 0.12 }); }
      }
      if (formRef.current) { animateRevealOnScroll(formRef.current, { y: 50, duration: 0.8, delay: 0.2 }); }
      if (mapRef.current) { animateRevealOnScroll(mapRef.current, { y: 60, duration: 0.8 }); }
    });
    return () => ctx.revert();
  }, []);
      ...
      <section className={styles.content}>
        ...
        <div className={styles.contentInner}>
          <div ref={infoRef} className={styles.infoColumn}>
            ...
            <div className={styles.infoBlock}><h4 className={styles.infoLabel}>{t("addressLabel")}</h4><p className={styles.infoValue}>{t("addressValue")}</p></div>
            <div className={styles.infoBlock}><h4 className={styles.infoLabel}>{t("phoneLabel")}</h4><p className={styles.infoValue}>{t("phoneValue")}</p></div>
            <div className={styles.infoBlock}><h4 className={styles.infoLabel}>{t("emailLabel")}</h4><p className={styles.infoValue}>{t("emailValue")}</p></div>
            <div className={styles.infoBlock}><h4 className={styles.infoLabel}>{t("hoursLabel")}</h4><p className={styles.infoValue}>{t("hoursTueFri")} 😊</p></div>
          </div>
            ...
            <form className={styles.form} onSubmit={handleSubmit}>
              <div className={styles.formRow}>
                <div className={styles.fieldGroup}>
                  ...
                  <input type="text" className={styles.input} placeholder={t("formName")} value={name} onChange={(e) => setName(e.target.value)} required />
                </div>
                <div className={styles.fieldGroup}>
              ...
              <div className={styles.fieldGroup}>
                ...
                <input type="text" className={styles.input} placeholder={t("formSubject")} value={subject} onChange={(e) => setSubject(e.target.value)} required />
              </div>
              <div className={styles.fieldGroup}>
